...
if (cartesiaApiKey) {
  ...
  console.warn('CARTESIA_API_KEY not found in environment variables. Speech synthesis will be disabled.');
}
...
export const speakAssistantMessage = async (text: string): Promise<void> => {
  ...
  try {
    ...
    const modelId = process.env.REACT_APP_CARTESIA_MODEL_ID || 'sonic-2';
    const voiceId = process.env.REACT_APP_CARTESIA_VOICE_ID || 'a0e99841-438c-4a64-b679-ae501e7d6091';
